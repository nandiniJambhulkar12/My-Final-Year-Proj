...
const Signup: React.FC = () => {
  const { signup, loginWithGoogle, sendPhoneOtp, verifyPhoneOtp, resetPhoneAuth, user } = useAuth();
  ...
  const [pwCheck, setPwCheck] = useState<{ id: string; pass: boolean; message: string }[]>([]);
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await axios.post(`${process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000'}/api/auth/register`, {
          email,
          name,
        });
      } catch (err) {
      ...
      <div className="card w-full max-w-md">
        ...
        <div className="flex mb-6 border-b border-gray-200 dark:border-gray-700">
          <button
            ...
            onClick={() => { setActiveTab('email'); setError(null); resetPhoneState(); }}
          >
            Email & Password
          </button>
          <button
          ...
          <form onSubmit={handleEmailSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="mt-2 text-sm">
                <div className="mb-1">Strength: <strong>{pwStrength || '—'}</strong></div>
                <ul className="text-xs space-y-1">
                  {pwCheck.map((r) => (
                    <li key={r.id} className={r.pass ? 'text-green-600' : 'text-gray-500'}>
                      {r.pass ? '✓' : '○'} {r.message}
                    </li>
                  ))}
                </ul>
            ...
            <button type="submit" className="btn-primary w-full" disabled={loading || !pwValid || password !== confirmPassword}>
        ...
        <div className="space-y-3">
          <button onClick={handleGoogle} className="btn-secondary w-full" disabled={loading}>
            Continue with Google
          </button>
        </div>
        <p className="mt-4 text-sm text-gray-600 dark:text-gray-300">
          Already have an account?{' '}
          <Link to="/login" className="text-primary-600">Log in</Link>
        </p>
      </div>
    </div>
  );
